...
if __name__ == '__main__':
    ...
    s = ttk.Style()
    s.configure('button_sunken',relief="groove")
    ...
    def encrypt_form():
        if en_count.get() == 0:
            ...
            aes_key_len = ttk.Label(aes_frame,text="Choose the mode of the key:")
            ...
            tt.create_Tooltip(len_choice,'Choose the mode of operation for AES operation')
            ...
            mes_loc_entered = ttk.Entry(mess_frame, width= 63, font=fontsize_12,textvariable = message_loc)
            mes_loc_entered.grid(column=0,row=1,padx=4,pady=12,ipady=7,sticky=tk.EW)
            ...
            pub_loc_entered = ttk.Entry(rsapub_frame,width= 63, font=fontsize_12,textvariable = rece_pub_loc)
            pub_loc_entered.grid(column=0,row=1,padx=4,pady=8,ipady=7,sticky=tk.EW)
            ...
            sav_file_entered = ttk.Entry(out_form, width= 63, font=fontsize_12, textvariable = sav_en_loc)
            sav_file_entered.grid(column=0,row=1,padx=4,pady=8,ipady=7,sticky=tk.EW)
            browse_button_pub = ttk.Button(out_form,text="..",command=browse_sav_file, width=3)
            browse_button_pub.grid(column=1,row=1,sticky=tk.E)

            mak_enve = ttk.Button(button_frame, text="Make an Envelope", width=10, command=_encrypt_mes)
    ...
    def browse_cryp():
        filename = filedialog.askopenfilename( title="Browse the encrypted file", filetypes=(("text files","*.txt"),("all files","*.*")))
    ...
    def _encrypt_mes():
        if message_loc.get() and rece_pub_loc.get()  and sav_en_loc.get():
            start = time.time()
            ...
            if file_check and directory_check:
                ...
                encryptedKeyContent = rsaEncrypt.encryptKey(rece_pub_loc.get(), aeskey)
    ...
    def _open_mes():
        ...
        if cryp_loc.get() and cryp_key_loc.get() and rece_priv_loc.get() and sav_de_lc.get():
            start = time.time()
            file_check = check_exist_file()
            ...
            if file_check and folder_check:
                ...
                decrypted_key = rsaDecrypt.readFromFileAndAuthenticateKey(cryp_key_loc.get(), rece_priv_loc.get())
                #print(decrypted_key)
                p1d = Process(target=aes_decrypt,args=(cryp_loc.get(),decrypted_key,out_path_decrypted_message,))
                p1d.start()
    ...
    def decrypt_form():
        if de_count.get() == 0:
            ...
            cryp_txt = "Browse the encrypted message file :"
            cryp_label = ttk.Label(crypfile_form, text=cryp_txt)
            fontsize_13(cryp_label)
            cryp_label.grid(column=0, row=0, padx=4, pady=12,sticky=tk.W)

            cryp_file_entered = ttk.Entry(crypfile_form, width=63, font=fontsize_10, textvariable=cryp_loc)
            cryp_file_entered.grid(column=0, row=1, padx=4, pady=12, ipady=7, sticky=tk.EW)
            ...
            cryp_key = "Browse the encrypted key and digest file : "
            ...
            browse_button_cryp = ttk.Button(crypfile_form, text="..", command=browse_cryp_key, width=3).grid(column=1, row=3,padx=8,pady=12,
                                                                                                         sticky=tk.W)
            rsa_file = "Browse the file containing the private key of the receiver (defined format) :"
            rsa_label = ttk.Label(priv_form, text=rsa_file)
            fontsize_13(rsa_label)
            rsa_label.grid(column=0, row=0, padx=4, pady=12,sticky=tk.W)

            priv_loc_entered = ttk.Entry(priv_form, width=63, font=fontsize_10, textvariable=rece_priv_loc)
            priv_loc_entered.grid(column=0, row=1, padx=4, pady=12, ipady=7, sticky=tk.EW)
            ...
            browse_button_priv = ttk.Button(priv_form, text="..", command=browse_priv_rece, width=3).grid(column=1, row=1,padx=8, sticky=tk.E)

            # rsa_file = "Browse the file containing the public key of the sender (defined format) :"
            # rsa_label2 = ttk.Label(priv_form, text=rsa_file)
            # fontsize_13(rsa_label2)
            # rsa_label2.grid(column=0, row=2, padx=4, pady=12, sticky=tk.W)
            #
            # pub_loc_entered = ttk.Entry(priv_form, width=63, font=fontsize_10, textvariable=send_pub_loc)
            # pub_loc_entered.grid(column=0, row=3, padx=4, pady=12, ipady=7, sticky=tk.EW)
            # tt.create_Tooltip(pub_loc_entered,
            ...
            # browse_button_pub = ttk.Button(priv_form, text="..", command=browse_pub_send, width=3).grid(column=1, row=3,padx=8,
            #                                                                                          sticky=tk.E)

            sav_file_label = ttk.Label(out_form_open, text="Browse the folder to save the output files :")
            fontsize_13(sav_file_label)
            sav_file_label.grid(column=0, row=1, padx=8, pady=8, sticky=tk.W)
            sav_file_entered = ttk.Entry(out_form_open, width=63, font=fontsize_12, textvariable=sav_de_lc)
            sav_file_entered.grid(column=0, row=2, padx=8, pady=8, ipady=7, sticky=tk.EW)
            tt.create_Tooltip(sav_file_entered,"Choose the folder you want to keep the output file.")
            browse_button_pub = ttk.Button(out_form_open, text="..", command=browse_sav_rece, width=3).grid(column=1, row=2,pady=8,padx=8,
                                                                                                           sticky=tk.E)
    ...
    def check_exist_file():
        if func_id.get() == 1:
            ...
            if os.path.isfile(message_loc.get()) and os.path.isfile(rece_pub_loc.get()):
                return True
            ...
            if os.path.isfile(cryp_loc.get()) and os.path.isfile(cryp_key_loc.get()) and os.path.isfile(rece_priv_loc.get()):
                return True
            elif not os.path.isfile(cryp_loc.get()):
                msg.showwarning('Warning', 'The Encrypted file doesn\'t exist.')
            elif not os.path.isfile(cryp_key_loc.get()):
                msg.showwarning('Warning', 'The Encrypted key file doesn\'t exist.')
